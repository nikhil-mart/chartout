package com.locolhive.chartout.helpers.paperonboarding;
...
import com.locolhive.chartout.helpers.paperonboarding.listeners.AnimatorEndListener;
import com.locolhive.chartout.helpers.paperonboarding.listeners.OnSwipeListener;
import com.locolhive.chartout.helpers.paperonboarding.listeners.PaperOnboardingOnChangeListener;
import com.locolhive.chartout.helpers.paperonboarding.listeners.PaperOnboardingOnLeftOutListener;
import com.locolhive.chartout.helpers.paperonboarding.listeners.PaperOnboardingOnRightOutListener;
import com.locolhive.chartout.helpers.paperonboarding.utils.PaperOnboardingEngineDefaults;
...
public class PaperOnboardingEngine implements PaperOnboardingEngineDefaults {
  ...
  /**
   * Calculate new position for pager without using pager's current position(like .getX()) this method allows to avoid incorrect position values while
   * animation of pager in progress
   *
  ...
  protected Animator createContentCenteringVerticalAnimation(View newContentText, View newContentIcon) {
    ...
    int newHeightOfContent =
        measuredContentIconHeight + measuredContentTextHeight + ((ViewGroup.MarginLayoutParams) mContentTextContainer.getLayoutParams()).topMargin;
  ...
  protected ImageView createContentIconView(PaperOnboardingPage PaperOnboardingPage) {
    final ImageView contentIcon = new ImageView(mAppContext);
    contentIcon.setImageResource(PaperOnboardingPage.getContentIconRes());
    FrameLayout.LayoutParams iconLP = new FrameLayout.LayoutParams(LayoutParams.MATCH_PARENT, LayoutParams.WRAP_CONTENT);
    iconLP.bottomMargin = dpToPixels(10);
